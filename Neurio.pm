...
###################################################################################################
=head2 add_Appliance - Add an appliance for a specific location

 Add an appliance for a specified location. 
 
   $Neurio->add_Appliance($location_id,$name,$label,$tags);
   
   The $name parameter must be one of a pre-defined list of appliances.
   At the moment this list is limited to:
     - air_conditioner
     - dryer
     - electric_kettle
     - heater
   ...
   This method accepts the following parameters:
     ...
     - $name         : name of the appliance - Required
     - $label        : label for the appliance - Optional
...
sub add_Appliance {
    my ($self,$location_id,$name,$label,$tags) = @_;
    
    # check if $location_id is defined
    if ((!defined $location_id) or (!defined $name)) {
      $self->printLOG("\nNeurio->add_Appliance(): \$location_id and \$name are required parameters\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'Neurio->add_Appliance(): \$location_id and \$name are required parameters';
      return 0;
    }
    ...
    $params->{'name'}       = $name;
    $params->{'label'}      = $label if defined $label;
    $params->{'tags'}       = $tags if defined $tags;

    my $url = $self->{'Appliances_url'};

    return $self->__process_post($url,$params);
}

###################################################################################################
=head2 tag_Event - Tag an appliance event for a specific appliance

 Tag an appliance event for a specified appliance.  
 
   $Neurio->tag_Event($event,%updateSimilar);
   
   This method accepts the following parameters:
     - $event         : a JSON structure for the event to be tagged - Required
       $updateSimilar : a boolean flag to indicate if similar events should b eupdated - Optional

 The event structure should contain the new appliance details already 
...
sub tag_Event {
    my ($self,$event,$updateSimilar) = @_;
    
    # check if $event is defined
    if ((!defined $event)) {
      $self->printLOG("\nNeurio->tag_Event(): \$event is a required parameter\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'Neurio->tag_Event(): \$event is a required parameter';
      return 0;
    }
    
    my $eventId = $event->{'id'};
    my $url     = $self->{'Appliances_Events_url'}."/".$eventId;
    
    if (defined $updateSimilar) {
      $url = $url . "?".$updateSimilar;
    }
    return $self->__process_put($url,$event);
}


###################################################################################################
=head2 tag_Event_confirm - Confirm that an event has been correctly tagged

 Confirm that an event has been correctly tagged.
 The isConfirmed parameter will be set to TRUE
 
   $Neurio->tag_Event_confirm($eventId);
   
   This method accepts the following parameters:
     - $eventId   : ID of the event to confirm
...
sub tag_Event_confirm {
    my ($self,$event) = @_;
    
    # check if $event is defined
    if ((!defined $event)) {
      $self->printLOG("\nNeurio->tag_Event_confirm(): \$event is a required parameter\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'tag_Event_confirm->tag_Event(): \$event is a required parameter';
      return 0;
    }
    
    my $eventId             = $event->{'id'};
    $event->{'isConfirmed'} = JSON::true;
    my $url                 = $self->{'Appliances_Events_url'}."/".$eventId."/confirmation";
    
    return $self->__process_post($url,$event);
}


###################################################################################################
=head2 tag_Event_unconfirm - Unconfirm an event that has previously been confirmed

 Unconfirm an event that has previously been confirmed
 The isConfirmed parameter will be set to FALSE
 
   $Neurio->tag_Event_unconfirm($eventId);
   
   This method accepts the following parameters:
     - $eventId   : ID of the event to unconfirm
...
sub tag_Event_unconfirm {
    my ($self,$event) = @_;
    
    # check if $event is defined
    if ((!defined $event)) {
      $self->printLOG("\nNeurio->tag_Event_unconfirm(): \$event is a required parameter\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'tag_Event_unconfirm->tag_Event(): \$event is a required parameter';
      return 0;
    }
    ...
    return $self->__process_delete($url,$event);
}


###################################################################################################
=head2 edit_Appliance - Edit appliance parameters

 Edit appliance parameters. 
 
   $Neurio->edit_Appliance($appliance_id,$name,$label,$tags);
   ...
   This method accepts the following parameters:
     - $appliance_id : id of the location - Required
     - $name         : name of the appliance - Required
     - $label        : label for the appliance - Optional
...
sub edit_Appliance {
    my ($self,$appliance_id,$name,$label,@tags) = @_;
    
    # check if $appliance_id is defined
    if (!defined $appliance_id || !defined $name) {
      $self->printLOG("\nNeurio->edit_Appliance(): \$appliance_id and \$name are required parameters\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'Neurio->edit_Appliance(): \$appliance_id and \$name are required parameters';
      return 0;
    }
    
    my $params;
    $params->{'name'}   = $name if defined $label;
    $params->{'label'}  = $label if defined $label;
    $params->{'tags'}   = \@tags;

    my $url = $self->{'Appliances_url'}."/" .$appliance_id;

    return $self->__process_put($url,$params);
}


###################################################################################################
=head2 delete_Appliance - Delete an existing appliance for a specific location

 Delete an existing appliance for a specified location. 
 
   $Neurio->delete_Appliance($applianceId);
   
   This method accepts the following parameters:
     - $applianceId  : id of the appliance to delete - Required
...
sub delete_Appliance {
    my ($self,$applianceId) = @_;
    
    # check if $applianceId is defined
    if (!defined $applianceId) {
      $self->printLOG("\nNeurio->delete_Appliance(): \$applianceId is a required parameters\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'Neurio->delete_Appliance(): \$applianceId is a required parameters';
      return 0;
    }
    
    my $url = $self->{'Appliances_url'}."/$applianceId";

    return $self->__process_delete($url);
}
...
###################################################################################################
=head2 update_Location - update information for a location

 Updates the Name, TimeZone or Energy Rate information for a particular location.
 
   $Neurio->update_Location($location_id, $name, $timezone, $energyRate);

   This method accepts the following parameters:
     - $location_id  : id of the location - Required
     - $name         : Name of the location - Optional
     - $timezone     : timezone of the location - Optional
     - $energyRate   : numeric energy rate of the location - Optional
...
sub update_Location {
    my ($self,$location_id,$name,$timezone,$energyRate) = @_;
        
    # make sure $location_id is defined
    if (!defined $location_id) {
      $self->printLOG("\nNeurio->update_Location(): \$location_id is a required parameter\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'Neurio->update_Location(): \$location_id is a required parameter';
      return 0;
    }
...
#    $params->{'locationId'} = $location_id;
    ...
    $params->{'timezone'}   = $timezone if defined $timezone;
    $params->{'energyRate'} = $energyRate if defined $energyRate;
    
    my $url = $self->{'Location_url'}."/$location_id";
    return $self->__process_patch($url,$params);
}
...
###################################################################################################
=head2 update_UserEmail - modify email address for a particular user

 Modifies email information for a particular user.
 
   $Neurio->update_UserEmail($user_id,$email,$password);

   This method accepts the following parameters:
     - $user_id  : id of the user - Required
     - $email    : new email for the user - Required
...
sub update_UserEmail {
    my ($self,$user_id,$email,$password) = @_;
    ...
    # make sure $user_id is defined
    if (!defined $user_id) {
      $self->printLOG("\nNeurio->update_UserEmail(): \$user_id is a required parameter\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'Neurio->update_UserEmail(): \$user_id is a required parameter';
      return 0;
    }
    
    my $url = $self->{'Users_url'}."/$user_id";
    my $info = $self->__process_post($url,$params);
...
###################################################################################################
=head2 update_UserPassword - modify password for a particular user

 Modifies password for a particular user.
 
   $Neurio->update_UserPassword($user_id,$oldPassword,$newPssword);

   This method accepts the following parameters:
     - $user_id     : id of the user - Required
     - $oldPassword : old password for the user - Required
...
sub update_UserPassword {
    my ($self,$user_id,$oldPassword,$newPassword) = @_;
    ...
    # make sure $user_id is defined
    if (!defined $user_id) {
      $self->printLOG("\nNeurio->update_UserPassword(): \$user_id is a required parameter\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'Neurio->update_UserPassword(): \$user_id is a required parameter';
      return 0;
    }
    
    my $url = $self->{'Users_url'}."/$user_id";
    my $info = $self->__process_put($url,$params);
...
###################################################################################################
=head2 fetch_Cycles_by_Sensor - Fetch information about cycles for your sensor ID

 Retrieves information about cycles for your sensor id.
 
   $Neurio->fetch_Cycles_by_Sensor($start,$end, $perPage, $page);

   This method accepts the following parameters:
      - $start        : yyyy-mm-ddThh:mm:ssZ - Optional
                        specified using ISO8601 format
      - $end          : yyyy-mm-ddThh:mm:ssZ - Optional
                        specified using ISO8601 format
      - $perPage      : number of results per page - Optional
 ...
 [
    {
        "id" : "2SMROBfiTA6huhV7Drrm1g",
        "groupId" : "0qX7nB-8Ry2bxIMTK0EmXw",
        "start" : "2014-03-21T20:28:32.000Z",
        "end" : "2014-03-21T21:28:32.000Z",
        "energy" : 12345,
        "averagePower" : 120,
        "significant" : false,
        "sensorId" : "0x123456789",
        "createdAt" : "2014-04-21T22:28:32Z",
        "updatedAt" : "2014-04-21T22:45:32Z",
        "upEdge" : {
            "id" : "2SMROBfiTA6huhV7Drrm1g",
            "sensorId" : "1cRsH7KQTeONMzjSuRJ2aw",
            "groupId" : "0qX7nB-8Ry2bxIMTK0EmXw",
            "start" : "2014-03-21T20:28:32Z",
            "end" : "2014-03-21T20:38:32Z",
            "peak" : "2014-03-21T20:38:32Z",
            "powerDelta" : 124,
            "direction" : 1,
            "significant" : false,
...
sub fetch_Cycles_by_Sensor {
    my ($self,$start,$end,$perPage,$page) = @_;
    
    my $url = $self->{'Cycles_url'}."?sensorId=".$self->{'sensor_id'};
...
###################################################################################################
=head2 fetch_Cycles_by_Sensor_Full - Fetch full information about cycles for your sensor ID

 Retrieves full information about cycles for your sensor id.
 
   $Neurio->fetch_Cycles_by_Sensor_Full($start,$end, $perPage, $page);
 ...
 [
    {
        ...
        "downEdge" : {
            ...
            "samples" : [
                {
                "id" : "2SMROBfiTA6huhV7Drrm1g",
                "timestamp" : "2014-03-21T20:35:32Z",
                "channelSamples" :
                    {
                        "name" : "edge2",
                        "power" : 159,
...
sub fetch_Cycles_by_Sensor_Full {
    my ($self,$start,$end,$perPage,$page) = @_;
    
    my $url = $self->{'Cycles_Full_url'}."?sensorId=".$self->{'sensor_id'};
...
###################################################################################################
=head2 fetch_Cycle - Fetch information about a particular cycle

 Retrieves information about a particular cycle
 
 A cycle_id can be requested by using the NeurioTools::get_cycle_ID 
 method which returns the cycle ID for the most recent cycle

   $Neurio->fetch_Cycle($cycle_id);

   This method accepts the following parameters:
      - $cycle_id : a cycle ID - Required
 ...
 Returns a Perl data structure on success
 {
    ...
    "downEdge" : {
        ...
        "sensorId" : "1cRsH7KQTeONMzjSuRJ2aw",
        "groupId" : "0qX7nB-8Ry2bxIMTK0EmXw",
        "start" : "2014-03-21T20:35:32.000Z",
        "end" : "2014-03-21T20:38:32.000Z",
        "peak" : "2014-03-21T20:35:32.000Z",
        "powerDelta" : 120,
        "direction" : -1,
        "significant" : false,
        "channelType" : "generation",
...
###################################################################################################
=head2 add_Cycle - Add a Cycle

 Add a cycle
 
   $Neurio->add_Cycle($location_id,$name,$label,$tags);
...
sub add_Cycle {
    ...
    # check if $location_id is defined
    if ((!defined $location_id) or (!defined $name)) {
      $self->printLOG("\nNeurio->add_Cycle(): \$location_id and \$name are required parameters\n\n");
      $self->{'last_code'}   = '0';
      $self->{'last_reason'} = 'Neurio->add_Cycle(): \$location_id and \$name are required parameters';
      return 0;
    }
...
###################################################################################################
=head2 fetch_Cycle_Groups_by_Sensor - Fetch information about cycle groups for your sensor ID

 Retrieves information about cycle groups for your sensor id.
 
   $Neurio->fetch_Cycle_Groups_by_Sensor();
 ...
 [
     {
         "id" : "2SMROBfiTA6huhV7Drrm1g",
         "applianceId": "---nrAGOQJatxUAw7xq9rQ",
         "attributes": {},
         "guesses": {},
         "createdAt" : "2014-04-21T22:28:32Z",
         "updatedAt" : "2014-04-21T22:45:32Z"
...
sub fetch_Cycle_Groups_by_Sensor {
    my $self = shift;
    my $url  = $self->{'Cycles_Groups_url'}."?sensorId=".$self->{'sensor_id'};;
...
###################################################################################################
=head2 fetch_Cycle_Group - Fetch information about a cycle groups for your sensor ID

 Retrieves information about a cycle group for your sensor id.
 
   $Neurio->fetch_Cycle_Group($cycle_group_id);

   This method accepts the following parameters:
     - $cycle_group_id : id of a cycle group - Required
...
sub fetch_Cycle_Group {
    my ($self,$cycle_group_id) = @_;
    
    my $url  = $self->{'Cycles_Groups_url'}."/";

    # if REQUIRED parameter is defined, add it
    if (defined $cycle_group_id) {
      $url = $url . "$cycle_group_id";
    }
...
###################################################################################################
=head2 fetch_Edges - Fetch information about edges

 Retrieves information about edges for a particular sensor.
 
   $Neurio->fetch_Edges($group_Id,$significant,$channelType,$start,$end);

   This method accepts the following parameters:
     - $group_Id    : id of a group - Optional
     - $significant : if edge is significant - Optional
     - $channelType : chennel type - Optional
     - $start       : yyyy-mm-ddThh:mm:ssZ - Optional
                      specified using ISO8601 format
     - $end         : yyyy-mm-ddThh:mm:ssZ - Optional
                      specified using ISO8601 format
...
###################################################################################################
=head2 fetch_Edge - Fetch information about an edges 

 Retrieves information about a particular edge for a particular sensor.
 
   $Neurio->fetch_Edge($edge_id);
     - $edge_id : id of a group - Required
...
###################################################################################################
=head2 fetch_Analytics - Fetch information about Neurio analytics 

 Retrieves information about Neurio analytics.
 
   $Neurio->fetch_Analytics($name,$perPage,$page);

   This method accepts the following parameters.
       - $name    : beginning of analytics name - Required
       - $perPage : number of results per page - Optional
       - $page    : page number to return - Optional
...
sub fetch_Analytics {
    my ($self,$name,$perPage,$page) = @_;

    my $url = $self->{'Analytics_url'}."?";
...
###################################################################################################
=head2 get_rateLimitRemaining - return the rate limit remaining

 Returns the number of requests remaining in the current rate limit window
 
   $Neurio->get_rateLimitRemaining();
...
sub get_rateLimitRemaining {
    my $self = shift;
    return $self->{'rateLimitRemaining'};
}

###################################################################################################
=head2 get_rateLimitLimit - return the maximum requests per hour

 Returns the maximum number of requests that the consumer is permitted to make per hour
 
   $Neurio->get_rateLimitLimit();

   This method accepts no parameters:
 
 Returns the maximum requests per hour
 
=cut

sub get_rateLimitLimit {
    my $self = shift;
    return $self->{'rateLimitLimit'};
}

###################################################################################################
=head2 get_rateLimitReset - return the time until limit is reset

 Returns the time at which the current rate limit window resets in milliseconds
 
   $Neurio->get_rateLimitReset();

   This method accepts no parameters:
 
 Returns the time until reset
 
=cut

sub get_rateLimitReset {
    my $self = shift;
